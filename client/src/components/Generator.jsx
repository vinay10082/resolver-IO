...
const Generator = ({title,apiLink}) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post(`http://localhost:8080/api/v1/openai/${apiLink}`, { text });
      console.log(data);
      setResponse(data);
    } catch (err) {
      console.log(error);
  ...
  return (
    <div class="mb-6 mx-auto w-96">
      <h1 class='font-bold text-center text-3xl mb-6'>{title}</h1>
      ...
      </form>
      {
        response?
        (
          <div class="container text-black p-5 mx-auto bg-gray-50 rounded-lg h-96 min-h-full">
          {response}
          </div>
        )
        :
